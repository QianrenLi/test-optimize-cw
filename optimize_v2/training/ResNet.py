...
class Residual(nn.Module):
    def __init__(self, in_channel, out_channel, use_1x1Conv=False, strides=1):
        super().__init__()
        self.conv1 = nn.Conv1d(in_channel, out_channel, kernel_size=3, padding=1, stride=strides)
        self.bn1 = nn.BatchNorm1d(out_channel)
        self.conv2 = nn.Conv1d(out_channel, out_channel, kernel_size=3, padding=1)
        self.bn2 = nn.BatchNorm1d(out_channel)

        if use_1x1Conv:
            self.conv3 = nn.Conv1d(in_channel, out_channel, kernel_size=1, stride=strides)
        else:
            self.conv3 = None
...
def resnet18(action_num):
    """ return a ResNet 18 object
    """
    return ResNet(BasicBlock, [2, 2, 2, 2], action_num)


def resnet34(action_num):
    """ return a ResNet 34 object
    """
    return ResNet(BasicBlock, [3, 4, 6, 3],action_num)
