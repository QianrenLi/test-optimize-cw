...
# from training.system2trainV2 import wlanDQNController
    ...
    if not multiple_IC:
        ...
        for device_name in ip_table.keys():
            ...
            if len(_depart_name) > 1:
                ...
                for protocol, ip in ip_table[device_name].items():
                    graph.associate_ip(device_name, protocol[0:3], ip)
    else:
...
def DQN_training_thread():
    ...
    while True:
        ...
        #     wlanController.store_params("%s/training/model/%s.pt" % (abs_path, experiment_name))
        is_network_use.set()
    wlanController.store_params("%s/training/model/%s.pt" % (abs_path, experiment_name))
    ...
    while control_times < time_limit:
        ...
        try:
            ...
            if state_ is not None:
                if is_line_search_policy:
                    his_fraction = this_throttle_fraction
...
def start_testing_threading(graph, ctl_prot):
    ...
    for device_name, links in graph.graph.items():
        for link_name, streams in links.items():
            ...
            prot, sender, receiver = link_name.split("_")
            ip_addr = graph.info_graph[sender][ctl_prot + "_ip_addr"]               ## since no p2p/lo is considered
            sock = ipc_socket(
                ip_addr,
        ...
        for _file_link in _lists:
            for comb in combinations(_lists, _ind):
                ...
                try:
                    ...
                    _set_manifest(_graph)
                    _graph.show()
                    start_testing_threading(_graph, "wlx")
                    plot_thread()
...
def local_training():
    ...
    wlanController.load_params("%s/training/model/%s.pt" % (abs_path, experiment_name))
    wlanController.load_memory(abs_path + "/training/saved_data/6-26-3-12-55-2023-06-27-00:50:41.npy")
    loss_saved = 0
    episode = 1000
    num_episode = 10
    counter_max = 50
...
def main(args):
    global experiment_name, wlanController, is_test, DURATION
    experiment_name = args.experiment_name
    if args.local:
        local_training()
        exit()

    if args.test or is_line_search_policy:
        is_test = True
        DURATION = 50
    else:
        is_test = False
    ...
    if args.load:
        wlanController.load_params("%s/training/model/%s.pt" % (abs_path, experiment_name))
        # wlanController.load_memory(abs_path + "/training/saved_data/6-26-2-12-55-2023-06-26-23:42:38.npy")
        wlanController.action_counter = 250
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--test", action="store_true", help= "start test" 
    )
    parser.add_argument("--local",action="store_true", help="start local training" )
